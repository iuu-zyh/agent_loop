...
| | 输入 tokens | 其中命中缓存 | 缓存率 |
|---|---|---|---|
| 第一次对话 | 15k | 7.3k | **49%** |
| 聊到第 12 轮 | 126.9k | 77.6k | **61%** |

**聊得越久，这个比例越高。** 也就是说，跟他处得越熟，前面那一大段固定的内容就越不需要重复付费、重复计算。
...
**Q：要花钱吗？**
mod 本身免费。AI 的钱是付给你自己选的模型服务的，不是付给作者。而且因为他用的是追加式对话、能吃到前缀缓存（聊到十几轮时缓存率能到 60% 左右），重复部分按缓存价计费，长期聊下来会比"每句话都重发全部上下文"便宜不少。
