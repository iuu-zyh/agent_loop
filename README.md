...
服务端校验同格 / 好感 / 战力。**攻击会触发战斗 UI，战后由游戏自然结算是否杀死**，无需单独的杀人指令。
...
所有动作工具在执行前都要过一层**独立于模型的校验**（在 C# 侧）：

| 校验 | 说明 |
|---|---|
| 好感阈值 | 关系还没到位，越界动作直接拒绝 |
| 关系互斥 | 已有道侣不能再结、已有师徒不能重复拜 |
| 同格 | `shuang_xiu` / `chuan_gong` 等要求面对面 |
| 战力 | 切磋 / 攻击的合理性 |
| 物资足额 | 赠送 / 买卖的道具与灵石是否真的够 |

不通过就**直接拒绝，并把原因回传给模型**——让它知道自己为什么没做成，而不是静默失败或编一个成功。
