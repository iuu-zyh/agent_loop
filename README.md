...
| 工具 | 类型 | 干什么 | 关键参数 |
|---|---|---|---|
| `inspect_unit` | 只读 | 查任意人物的分类档案 | `unit_id` / `target`、`classes`、经历分页、背包裁剪 |
| `search_units` | 只读 | 按条件在全图找人 | `filters`：姓名 / 关系 / 境界 / 宗门 / 地区 / 种族 / 性别 |
| `query_world` | 只读 | 了解世界全局 | `topic`：大事 / 排行 / 地点 / 宗门 / 州名 |
| `social_relation` | 动作 | 关系与好感的增减 | `op`（13 种）、`value`、`ask_text` |
| `movement` | 动作 | 召唤 / 传送 / 前往某地等候 | `op`、`destination`、`region` |
| `world_ai_action` | 动作 | 切磋 / 攻击 / 双修 / 论道 / 邀约 / 传功 | `op`、`skill` |
| `economy_item` | 动作 | 赠予道具或灵石 | `items[]`、`via` |
| `trade` | 动作 | 牵线买卖（买卖双方都可指定） | `seller`、`buyer`、`item`、`count`、`price` |
| `item_acquire` | 动作 | 偷窃 / 讨要 | `op`、`item_name`、`count` |

前三个是**只读**工具，后六个会**改变世界**。完整参数与取值范围见[第十三节 · 工具参考](#十三工具参考)。
...
**服务端会复核。** 关系变更、行动、交易在执行前都要过一层独立检查：关系是否冲突、人在不在同一格、货物灵石是否足额。不通过就直接拒绝，并把原因告诉他。

> 注意：**好感门槛不在服务端校验范围内**——关系还没到位时该不该越界，靠他自己在人设里把握。详见[工具参考](#十三工具参考)末尾。
...
在配置面板的「提示词」页签就能直接改：左边选文件，右边编辑，保存。**下一句话就生效，不用重启游戏。** 还有个「新建人设」按钮，点一下就用当前正在对话的 NPC 名字建一份新的。
...
不用你去找他。每天有一定概率，某个跟你有交情的 NPC 会主动传讯过来，屏幕角落会弹出一条横幅提醒你。

> **横幅只是提醒，点它不会打开对话。** 消息存在通讯录里，从那里点开才看得到。
...
**找人也很方便。** 主界面 HUD 上有一枚「**传**」按钮，点开是通讯录：有交情的 NPC 都在里面，按名字排序，显示宗门、境界、和你的关系与好感。谁有新消息会亮红点，**横幅提醒的那条消息也在这里**：
...
- **主动找人**：点主界面 HUD 上的「**传**」按钮打开通讯录，点某一行
...
打开 AI 设置 →「大模型」页 →【模型与生成】→ 填入接口地址、API Key、模型名 → 点「**测试连接**」。
...
**Q：他会不会乱改我的关系、乱拿我的东西？**
不会。影响你的动作都要经过服务端检查（同格 / 关系状态冲突 / 物资足额），并且**必须你在确认窗里点同意**才会发生。你点拒绝，这件事就是没发生。
...
**动作二次校验** —— 模型给出的动作不直接执行。C# 侧会独立复核**关系状态互斥**、**同格**、**物资是否足额**，不通过就拒绝并回传原因。好感与战力不做服务端校验，由模型自己判断。
...
九个工具的参数契约由 **`tools/schemas.py` 唯一定义**——模型看到的就是这份 JSON Schema。
执行落地在 `csharp/ToolExecutor.cs`，结果渲染在 `tools/text_render.py`，
三者的一致性由 `tests/test_tool_contract_consistency.py` 机械校验（无死参数 / 无隐藏参数 / 枚举全覆盖 / 渲染层读的键 C# 必须真的发）。

**两组工具，两条规则：**

| | 工具 | 玩家忙碌时（战斗中 / 开着别的界面 / 有剧情弹窗） |
|---|---|---|
| **只读** | `inspect_unit` `search_units` `query_world` | ✅ 仍然可用——查询不打扰你 |
| **动作** | 其余六个 | ❌ 一律禁用——只有言语和只读可用 |

工具列表的排序即调用优先级：**查询 → 社交 → 位移 → 行动 → 经济 → 索取**。
...
查询指定人物的分类档案。`classes` 可多选需要的数据块（默认 `brief`），**没有笼统的 `full`**。

| 参数 | 类型 | 默认 | 说明 |
|---|---|---|---|
| `unit_id` | string | — | **优先用这个**。精确指定，填 `relationships` 或 `search_units` 结果里的 `unit_id`（形如 `Xs6JDI`） |
| `target` | string | — | 人物真名，拿不到 `unit_id` 时才用。查玩家填「玩家」段里的姓名，查自己填自己的名字 |
| `classes` | array | `["brief"]` | 要取的数据块，取值见下 |
| `log_page` | int ≥1 | 1 | 每页 5 条，**第 1 页 = 最新**（经历按新→旧排列） |
| `log_filter` | enum | `all` | `all` 重要+常规 / `important` 仅重要（突破、习法等大事）/ `regular` 仅常规（论道、双修、赠礼等日常） |
| `log_since_month` | int ≥1 | — | 只返回该账面月（含）之后的经历 |
| `log_until_month` | int ≥1 | — | 只返回该账面月（含）之前的经历 |
| `inventory_top` | int 1–50 | 3 | 背包分类内展示价值最高的前 N 种（总种类 ≤12 时自动全展示） |
| `inventory_all` | bool | false | `true` = 全量展示不裁剪（大背包慎用；找特定低价值道具时才用） |

`classes` 取值：

| 值 | 内容 |
|---|---|
| `brief` | 基础身份：境界 / 宗门 / 位置 / 关系 / 好感 / 气运 |
| `stats` | 全量属性（含加成） |
| `abilities` | 功法中文名 |
| `inventory` | 储物：灵石 / 分类背包 / 装备 |
| `relationships` | 关系网（每人带 `unit_id`，可再喂回 `unit_id` 参数） |
| `logs` | 人生经历分页 |

> **指人二选一**：同时给出 `unit_id` 与 `target` 时以 `unit_id` 为准，不再按名字匹配。
> 八荒重名是常态——按名查询命中多人时会返回 `ambiguous` 提醒并列出候选，那时模型会如实说"我分不清"。
> `log_*` / `inventory_*` 参数**只在选中对应块时生效**。
> 经历分页的账面月换算：`N年M月 = (N-1)*12 + M`（如 2年1月 = 13）；圈定范围时**先过滤再分页**。

---

#### `search_units` —— 全图找人

按过滤条件在全图搜索。结果按好感降序，**每行带 `unit_id`**，可直接填进 `inspect_unit`。

| 参数 | 类型 | 说明 |
|---|---|---|
| `filters` | object | **必填**。只填有把握确定的值——不确定的不要填。空字典 = 返回全图与玩家好感最高的前 10 |

`filters` 的子键（全部可选）：

| 键 | 类型 | 说明 |
|---|---|---|
| `keyword` | string | 姓名 / 关键字模糊匹配 |
| `relation` | enum | 与玩家的关系：`亲族` `好友` `道侣` `师徒` `结义` `好感` `陌生` |
| `realm` | string | 境界，如 `金丹` / `元婴` |
| `sect` | string | 宗门名 |
| `region` | string | 地区，如 `永宁州` / `雷泽` |
| `race` | enum | `人族` / `魔族` |
| `sex` | enum | `男` / `女` |
...
| 参数 | 类型 | 说明 |
|---|---|---|
| `topic` | enum | **必填**。`events` / `rankings` / `places` / `sects` / `region` |
| `cat` | enum | 仅 `topic=places`：`城镇` `宗门` `突破材料` `器灵材料`。省略时只回城镇+宗门 |
| `region` | string | 仅 `topic=places/sects`：按州过滤；不传返回全部 |
| `board` | enum | 仅 `topic=rankings`：`power` 战力 / `reputation` 声望 / `beauty` 魅力 / `money` 灵石 |
| `top` | int 1–200 | 仅 `topic=rankings`：返回前 N 名，默认 10 |
| `count` | int 1–120 | 仅 `topic=events`：按月志倒序取最近 N 条，默认 12 |
...
|---|---|
| `events` | 近期天下大事（世界月志） |
| `rankings` | 天下排行榜（战力 / 声望 / 魅力 / 灵石） |
| `places` | 可去地点（城镇 / 宗门 / 突破材料 / 器灵材料）——也是 `movement` 的 `travel` 目标目录 |
| `sects` | 宗门概览（名号 / 层级 / 存亡被占 / 宗主 / 宗旨气运） |
| `region` | 世界地区（州）名列表 |

> `places` / `sects` 结果可能很多，应先传 `region` 只查目标州（州名见世界背景，或先 `topic=region`）。

---

### 动作工具（6）

> 六个动作工具的**发起方恒为当前对话的 NPC、对象恒为玩家**，因此都不需要传 `target`。

#### `social_relation` —— 关系与好感

服务端会二次校验**关系状态互斥**（已是道侣不能再结、已有师徒不能重复拜等）。**好感阈值不校验**。

| 参数 | 类型 | 说明 |
|---|---|---|
| `op` | enum | **必填**，13 种，取值见下 |
| `value` | int 1–10 | 仅 `add_intim` / `reduce_intim` 需要 |
| `ask_text` | string | 可选。关系确认窗的正文——NPC 的**第一人称台词**（求婚词 / 结义词 / 拜师词 / 绝交词等）。仅关系类 `op` 有效，`add/reduce_intim` 会忽略；须符合人设与语境的古风口吻，过长会被截断；不传则用默认文案 |

`op` 取值：

| 类别 | 建立 | 解除 |
|---|---|---|
| 好感 | `add_intim`（+1~10） | `reduce_intim`（−1~10） |
| 道侣 | `jie_yuan` | `jie_chu_jie_yuan` |
| 夫妻 | `marry` | `divorce` |
| 师徒 | `bai_shi`（拜你为师） / `shou_tu`（收你为徒） | `jie_chu_bai_shi` / `jie_chu_shou_tu` |
| 结义 | `jie_yi` | `jie_chu_jie_yi` |
| 义父母 | `ren_yi_fu_mu` | — |
...
| 参数 | 类型 | 说明 |
|---|---|---|
| `op` | enum | **必填**。`summon` 召唤玩家到 NPC 处 / `teleport` NPC 传送到玩家处 / `travel` NPC 前往指定地点并在当地等候 |
| `destination` | string | 仅 `travel` 需要：城镇 / 宗门名（来自 `query_world topic=places`） |
| `region` | string | 仅 `travel` 可选：区域名消歧（各州分宗常同名） |

---

#### `world_ai_action` —— 对玩家发起行动

服务端校验**同格**。**攻击会触发战斗 UI，战后由游戏自然结算是否杀死**，无需单独的杀人指令。

| 参数 | 类型 | 说明 |
|---|---|---|
| `op` | enum | **必填**。`spar` 切磋 / `attack` 攻击 / `shuang_xiu` 双修 / `lun_dao` 论道 / `yao_yue` 邀约 / `chuan_gong` 传功 |
| `skill` | string | 可选，仅 `chuan_gong` 用到。省略时由玩家在游戏原生功法面板自行选择（**推荐**）；要指定某本时才填功法 ID 或名称 |

> **同格要求**：`shuang_xiu` 与 `chuan_gong` 须同处一地；`lun_dao` 与 `yao_yue` 属言语往来，可远程。
> `spar` 会弹原生确认窗，**等玩家应战或婉拒之后才返回结果**——模型届时应按玩家的实际选择说话，而不是预设一个结局。

---

#### `economy_item` —— 赠予

赠予道具或灵石给玩家。`items` 可多种，**灵石与道具可混送**。

| 参数 | 类型 | 默认 | 说明 |
|---|---|---|---|
| `items` | array | — | **必填**。赠送清单 |
| `items[].item_name` | string | — | **必填**。道具中文名（来自 `inspect_unit` 的 `inventory`）或 `灵石` |
| `items[].count` | int 1–99999 | 1 | 数量 |
| `via` | enum | `direct` | 送达方式，**仅影响纯灵石**赠送的确认窗与结果文案（含道具时道具恒走原版赠送 UI）。`direct` 按同格/异地自动措辞 / `letter` 信件赠达 |

> 赠予前应先 `inspect_unit(classes=["inventory"])` 确认 NPC 自己的背包。
> **纯灵石**赠送弹确认窗（拒收则不送）；**含道具**时道具走原版赠送 UI、灵石随之即时到账。
> 异地也可赠；**数量不足时全送**，并在结果的 `actual` 里说明。

---

#### `trade` —— 牵线买卖

把卖方背包里的一件道具卖给买方，买方付灵石给卖方。**买卖双方都由模型指定**（玩家或任意 NPC 均可），不限当前对话对象。

| 参数 | 类型 | 默认 | 说明 |
|---|---|---|---|
| `seller` | string | — | **必填**。卖方真名（出货方） |
| `buyer` | string | — | **必填**。买方真名（付灵石方） |
| `item` | string | — | **必填**。道具中文名；**不能填灵石** |
| `price` | int 0–99999999 | — | **必填**。买方付给卖方的灵石**总额**（整笔价，不是单价）；`0` = 无偿赠予 |
| `count` | int 1–9999 | 1 | 成交数量 |

> 会弹确认窗，**玩家点「成交」才真正转移**。
> 数量必须**整笔满足**：卖方存货不足或买方灵石不够都**整笔失败**（不像 `economy_item` 有多少送多少）——
> 想只成交一部分，得改小 `count` 重发。
...
| 参数 | 类型 | 默认 | 说明 |
|---|---|---|---|
| `op` | enum | — | **必填**。`steal_item` 偷窃 / `ask_for` 讨要 |
| `item_name` | string | — | **必填**。道具中文名（来自 `inspect_unit` 的 `inventory` 的 `props`），限普通道具 |
| `count` | int 1–9999 | 1 | 仅 `op=ask_for` 时传入（索要数量）；`steal_item` **不要传** |

> `steal_item` 游戏原生没有数量参数，**恒为整栈偷取**（同名多栈时一次偷一栈），**被发现会掉好感**。
> `ask_for` 可指定数量，限普通道具，**灵石不适用**。
> 相比偷窃，讨要温和得多——所以在人设和剧情合理的时候，NPC 会更倾向于开口讨要。

---

### 二次校验：工具不是想调就能调

动作工具在执行前会过一层**独立于模型的校验**——它不信任模型给的参数，自己再查一遍游戏状态。

**C# 侧实际会拦的：**

| 校验 | 说明 | 覆盖 |
|---|---|---|
| **同格** | 要求面对面的动作，异地直接拒绝 | `spar` `attack` `shuang_xiu` `chuan_gong` `marry` |
| **关系状态互斥** | 已是道侣不能再结、已有师徒不能重复拜、求婚前须先结缘…… | `social_relation` 全部关系类 op |
| **物资足额** | 见下表（两组工具语义不同） | `trade` / `economy_item` |

**物资足额的两组语义**——这个区别很重要：

| 工具 | 不够时怎么办 |
|---|---|
| `trade` | **整笔失败**。卖方存货不足或买方灵石不够都直接拒绝，并提示"改小 count 重发"；不可转手的道具（绑定/任务/装备中）也会被拦。还带**竞态回滚**：预校验通过后背包若变了，整笔作废、道具退回卖方 |
| `economy_item` | **有多少给多少**。只送得出部分就在结果的 `actual` 里说明；全不足才报错 |

**C# 侧不做、交给模型的：**

| 项 | 现状 |
|---|---|
| **好感阈值** | ❌ **服务端不校验**。关系还没到位时该不该越界，由模型按人设与剧情自己判断 |
| **战力比较** | ❌ 不校验。切磋 / 攻击是否合理，同样交给模型 |

> 写人设时值得注意这一点：**好感不够时不会有人替你拦**。
> 想让某个 NPC 矜持些、别一见面就求婚，得把这条写进他的人设里。

拒绝时会把**原因回传**给模型——让它知道自己为什么没做成，而不是静默失败或编一个成功。

真正影响玩家的动作还会再弹一层**确认窗**：你点同意才会发生。
