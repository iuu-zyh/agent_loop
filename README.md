...
3. **游戏原生对话 / 剧情窗**：里面会出现「AI 对话」选项，点开就是。这类窗口（包括 NPC 主动传讯、寻仇追杀找上门）会把屏幕上那句话原样交给模型，你直接接着聊，不用自己复述背景；原生选项本身的效果不受影响。
...
**他找上门的时候，不用你复述背景。** NPC 主动找你、寻仇追杀这类原生剧情 / 事件窗里，屏幕左侧会多出一枚「AI 对话」按钮：点它会把屏幕上那句话**原样**交给模型（取的就是界面上显示的成品句，不做推断），他按自己的人设接住这个话头，你直接接着聊。

![原生事件窗里多出的「AI 对话」按钮](docs/images/21-auto-context.png)

![点完按钮进到对话窗，他接着刚才那件事说下去](docs/images/22-auto-context-chat.png)

几点说明：

- 这枚按钮**不改变原生剧情**：原来的选项、结算照旧，被寻仇时该打起来还是会打起来，它只是多开一个入口。
- 语义是**中性**的：同一枚按钮既服务"他主动找上门"，也服务"你点了闲聊"——只把交互内容交给模型，不预设是谁主动。
- 没有 NPC 的系统公告类剧情不会出现这枚按钮。
